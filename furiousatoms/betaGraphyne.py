...
class Ui_polymer(QtWidgets.QMainWindow):
    ...
    def polymer_builder_callback(self):
        ...
        fname_monomer = 'C:/Users/nasim/Downloads/NewNS/betaGraphyne_unitcell.pdb'
    ...
    def polymer_builder(self, fname_monomer):
        ...
        distance_2 = max(pos[:, 0]) - min(pos[:, 0])
        distance = np.linalg.norm(pos[4]-pos[11]) + 1.4
        nx = load_file.dimensions [3]
        ny= load_file.dimensions [4]
        nz= load_file.dimensions [5]

        # load_file.dimensions = [distance, distance_2, distance, nx, ny, nz]9.740    9.485
        load_file.dimensions = [distance  ,  9.485-1.285, distance, nx, ny, nz]
